...
export function Heading_1({ children, className }: Props) {
  return <h1 className={`${SuisseIntl.className} text-heading ${className}`}>{children}</h1>;
}
...
export function Heading_3({ children, className }: Props) {
  return <h3 className={`${SuisseIntl.className} !text-heading ${className}`}>{children}</h3>;
}
